...
- **lane 並行 / デタッチ実行**: `RUNNER_LANE` で repo ごとに実行レーンを分離し、別 repo の作業を並行ドレインできる。`long-run` ラベルの Issue はデタッチ実行され、長時間タスクでもロックを占有しない。詳細は [共通実行キューとログ](docs/runner-queue.md) を参照。
