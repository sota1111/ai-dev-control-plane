...
- `[opus]` / `[sonnet]`（= `CLAUDE_MODEL` を短い名前に正規化, 既定 `opus`）: orchestrator
  （Claude Code 本体）のナレーション・ツール呼び出し。`CLAUDE_MODEL` が `claude-opus-4-8` /
  `opus` / `claude-sonnet-4-6` / `sonnet` / `haiku` のいずれでも `[opus]` / `[sonnet]` /
  `[haiku]` に揃う。例: `[opus] [Bash] git status`、`[sonnet] Codex is still running...`。
- `[codex]`: 実際に起動した Codex CLI の出力（`== Codex CLI` バナー由来）。
- `[agy]`: 実際に起動した Antigravity CLI（`agy`）の出力（`== Antigravity CLI` バナー由来）。
