...
7. **【恒久・人間可読の統合記録】\`docs/ai/STRATEGY_AND_ROADMAP.md\`（target repo）を作成/更新する（必須）**。
   機械可読な台帳とは別に、**人間が1枚で「これまでの試行錯誤・結果・今後の方針」を読める単一の真実源**を維持する。
   無ければ全履歴（台帳・過去申し送り・cycle_analysis）から新規作成、あれば当サイクルの結果を追記して現状と方針を更新。
   節構成: \`## 0. コンペの性質・制約・評価指標\` / \`## 1. 現状サマリ（champion・best スコア・実LB順位・最終更新日）\` /
   \`## 2. 試行錯誤の履歴と結果（フェーズ別・軸ごとの要点／rejected 理由／到達した壁）\` /
   \`## 3. 確定した結論（手法クラスの天井・パラダイム・律速）\` / \`## 4. 今後の方針（次の軸・選んだ方向とマイルストーン）\` /
   \`## 5. 参照\`。**古い記述は消さず追記・更新**（判断の変遷が追えること）。統合 PR に含める。
